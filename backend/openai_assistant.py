...
class OpenAIAssistant:
    def __init__(self):
        ...
        self.cat_api = CatAPI()
        self.thread = openai.beta.threads.create() # creates a thread
        ...
        while run.status != "completed":
            ...
            if run.status == "requires_action":
                for tool_call in run.required_action.submit_tool_outputs.tool_calls: # checks if any tools were needed
                    if tool_call.function.name == "get_cat": # checks if the "get_cat" tool is used
                        try:
                            arguments = json.loads(tool_call.function.arguments)
                            breed = arguments.get('breed')
                            limit = arguments.get('limit', 1)
                            cat_images = self.cat_api.get_cats(breed, limit)
